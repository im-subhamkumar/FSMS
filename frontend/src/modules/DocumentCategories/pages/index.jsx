        ...
        <div className="h-full flex flex-col space-y-6 animate-in fade-in duration-300">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1 pl-11">Manage classification settings and expiry policies for all system documents.</p>
                </div>
                <button 
                                ...
                                {categories.length === 0 ? (
                                    ...
                                    <tr key={cat.id} className="hover:bg-slate-50/80 transition-colors group">
                                        <td className="px-6 py-4">
                                            ...
                                            <div className="text-xs text-slate-400 mt-0.5">ID: {cat.id}</div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <span className="text-slate-600 text-sm max-w-xs block truncate" title={cat.description}>
                                        ...
                                        <td className="px-6 py-4">
                                            {cat.requiresExpiry ? (
                                                <div className="flex flex-col gap-1">
                                                    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-amber-100 text-amber-800 w-fit">
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden flex flex-col">
                        <div className="flex justify-between items-center p-6 bg-slate-900 text-white">
                            ...
                            <button onClick={handleCloseForm} className="p-2 hover:bg-slate-800 rounded-full transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    className="w-full border border-slate-300 rounded-lg p-2.5 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all resize-none h-20"
                            ...
                            <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-4">
                                <label className="flex items-start gap-3 cursor-pointer group">
                                    ...
                                    <div>
                                        <span className="block text-sm font-semibold text-slate-900 group-hover:text-blue-600 transition-colors">Tracks Expiry Dates</span>
                                        <span className="block text-sm text-slate-500 mt-0.5">Require an expiry date when uploading these documents.</span>
                                    </div>
                                </label>

                                {formData.requiresExpiry && (
                                    <div className="pl-7 pt-2 animate-in slide-in-from-top-2 fade-in duration-200">
                                        <label className="block text-sm font-medium text-slate-700 mb-1">Warning Threshold (Days)</label>
                                        <div className="flex items-center gap-2">
                                            <input 
                                                ...
                                                className="w-24 border border-slate-300 rounded-lg p-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                                            />
                                            <span className="text-sm text-slate-500">Days before expiry to show warnings</span>
                                        </div>
                                    </div>
                                )}
                            </div>
