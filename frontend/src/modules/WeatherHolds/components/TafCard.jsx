...
export default function TafCard({ tafData, error, isLoading }) {
  ...
  const renderContent = () => {
    const placeholderClass = "text-center p-6 text-sm font-medium text-slate-500 dark:text-slate-400 bg-white/50 dark:bg-slate-700/50 border border-slate-200 dark:border-slate-600 rounded-xl";
    ...
    return tafData.fcsts.map((f, idx) => {
      ...
      return (
        <div key={idx} className="flex flex-col sm:flex-row gap-3 p-3 bg-white/40 dark:bg-slate-700/30 border border-slate-200/50 dark:border-slate-600/50 rounded-xl transition-colors hover:bg-white/60 dark:hover:bg-slate-700/50">
          <div className="font-mono text-[11px] font-bold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 px-2 py-1 rounded w-fit sm:w-28 shrink-0 border border-blue-500/10 dark:border-blue-500/20 flex items-center justify-center">{timeLabel}</div>
          <div className="text-sm font-semibold text-slate-700 dark:text-slate-300 leading-relaxed">{parts.join(' · ') || 'No changes'}</div>
        </div>
      );
    });
  };
  ...
  return (
    <div className="bg-white/70 dark:bg-slate-800/60 backdrop-blur-xl border border-blue-500/20 dark:border-slate-700/50 p-5 mb-4 rounded-2xl shadow-lg relative z-10">
      <div className="flex items-center gap-2 pb-3 mb-4 border-b border-slate-200/50 dark:border-slate-700/50">
        ...
        <span className="font-bold text-slate-800 dark:text-white">TAF Forecast</span>
        <span className="ml-auto text-xs font-semibold px-2.5 py-1 bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-500 dark:text-slate-400 rounded-md shadow-sm">{badgeText}</span>
      </div>
