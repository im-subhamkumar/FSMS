...
export default function Header({ onSearch, isLive, stationId }) {
  ...
  return (
    <header className="relative z-50 flex flex-col md:flex-row justify-between items-center bg-white/70 backdrop-blur-lg border-b border-blue-500/20 px-6 py-4 mb-6 shadow-sm">
      <div className="flex items-center gap-4">
        <div className="text-3xl text-blue-600 bg-blue-50 w-12 h-12 flex items-center justify-center rounded-xl shadow-sm">✈</div>
        <div>
          <div className="font-bold text-xl text-slate-800 tracking-tight">FSMS Weather</div>
          <div className="text-xs text-slate-500 font-medium uppercase tracking-wider">Flight School Management System</div>
        </div>
      </div>
      <div className="flex-1 w-full md:w-auto my-4 md:my-0 md:mx-8 max-w-lg" ref={containerRef}>
        <form className="relative flex w-full tracking-wide z-50" onSubmit={handleSearch}>
          ...
          <input
            className="w-full pl-10 pr-32 py-2.5 bg-white border border-slate-200 rounded-xl font-mono text-sm shadow-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 transition-all uppercase placeholder:normal-case relative z-50"
            type="text"
          ...
          {isFocused && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white/90 backdrop-blur-xl border border-slate-200/50 rounded-xl shadow-2xl overflow-hidden max-h-64 overflow-y-auto z-[60]">
              {filteredAirports.length === 0 ? (
                <div className="px-4 py-3 text-sm text-slate-500 italic">No airports found</div>
              ) : (
                filteredAirports.map(airport => (
                  <div
                    key={airport.icao}
                    className="px-4 py-2 hover:bg-blue-50 cursor-pointer flex justify-between items-center transition-colors border-b border-slate-100 last:border-0"
                    onClick={() => handleSelectAirport(airport.icao)}
                  >
                    <div className="flex flex-col">
                      <span className="font-bold text-slate-700 text-sm">{airport.icao}</span>
                      <span className="text-[11px] text-slate-500 uppercase tracking-widest">{airport.location}</span>
                    </div>
                    <span className="text-xs font-semibold text-slate-600 truncate max-w-[140px] sm:max-w-xs text-right">{airport.name}</span>
                  </div>
                ))
              )}
            </div>
