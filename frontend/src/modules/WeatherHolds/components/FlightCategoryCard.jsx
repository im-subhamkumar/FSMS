...
export default function FlightCategoryCard({ category, stationId, obsTime, stationName }) {
  ...
  const baseCardClass = "flex flex-col items-center justify-center p-8 bg-white/70 backdrop-blur-xl border border-blue-500/20 rounded-2xl shadow-lg relative overflow-hidden text-center";

  if (!stationId) {
    return (
      <div className={baseCardClass}>
        <div className="font-mono text-3xl font-black text-slate-400 tracking-wider mb-4">—</div>
        <div className={`text-2xl font-black px-8 py-2 rounded-full border-2 mb-4 tracking-widest ${catColors['UNKNOWN']}`}>Enter ICAO</div>
        <div className="text-sm font-medium text-slate-400 bg-white/50 px-4 py-1.5 rounded-lg border border-slate-200 mt-2">Awaiting search...</div>
      </div>
    );
  }
  ...
  return (
    <div className={baseCardClass}>
      <div className="font-mono text-4xl font-black text-slate-800 tracking-widest mb-5">{stationId}</div>
      <div className={`text-2xl font-black px-10 py-2.5 rounded-full border-2 mb-5 tracking-widest ${badgeStyle}`}>{category || '—'}</div>
      <div className="text-sm font-medium text-slate-600 bg-white/60 px-5 py-2 rounded-lg border border-slate-200/60 shadow-sm mt-2">{obsTime ? `Observed: ${formattedTime}` : formattedTime}</div>
      {stationName && <div className="text-xs font-semibold text-slate-400 uppercase tracking-widest mt-4">{stationName}</div>}
    </div>
  );
}
