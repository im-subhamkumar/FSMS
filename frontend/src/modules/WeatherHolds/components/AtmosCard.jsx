...
export default function AtmosCard({ atmosData, error, isLoading }) {
  const renderContent = () => {
    const placeholderClass = "text-center p-6 text-sm italic text-slate-400 bg-white/40 rounded-xl border border-dashed border-slate-400/40";
    ...
    if (error) return (
      <div className={`${placeholderClass} !border-amber-500/40`}>
        ...
        <div className="text-amber-500 font-semibold">Weather Service Error</div>
    ...
    return items.map((i, idx) => (
      <div key={idx} className="bg-white/40 border border-blue-500/10 rounded-xl p-3 text-center shadow-sm">
        <div className="text-[11px] font-bold text-slate-500 uppercase tracking-widest mb-1">{i.lbl}</div>
        <div className="text-lg font-extrabold text-blue-500 mb-0.5">{i.val}</div>
        <div className="text-[10px] text-slate-400 uppercase">{i.sub}</div>
      </div>
    ));
  };
  ...
  return (
    <div className="bg-white/70 backdrop-blur-xl border border-blue-500/20 p-5 mb-4 rounded-2xl shadow-lg relative z-10">
      <div className="flex items-center gap-2 pb-3 mb-4 border-b border-slate-200/50">
        ...
        <span className="font-bold text-slate-800">Atmos Data</span>
        <span className={`ml-auto text-[10px] font-bold px-2 py-0.5 rounded shadow-sm text-slate-600 ${provider === 'MET Norway' ? 'bg-blue-100' : 'bg-green-100'}`}>
          {provider}
        </span>
      </div>
